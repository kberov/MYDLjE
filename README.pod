...
MYDLjE (pronounced [maidalji]/[майдължи]) is a lightweight
...
=item * Apache2 with mod_cgi (standard) or (better/faster)
 mod_fcgi/mod_fastcgi;
...
Almost all (even the cheapest) hosting providers meet the system
...
=item 2 Create a database using the example found in
 I<conf/mysql.schema.sql> or use the tool (PHPMyAdmin) provided by your
 hosting provider

=item 3 Use your browser to access B<index.xhtml> or setup via commandline -
 see L</SETUP>.

=item 4 Check the system and troubleshoot :) if needed. Look at the apache
...
To setup your system <a href="index.xhtml">go to index.xhtml</a>. Make sure
 ...
 system. Follow the steps and you should be fine.

=end html

Many programmers prefer using a terminal. To set up the system this way,
 open a terminal and go to the MYDLjE root directory:
