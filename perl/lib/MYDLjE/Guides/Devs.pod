...
Current language is restored from C<$c-E<gt>msession('language')> or 
C<$c-E<gt>req-E<gt>param('language')> is used. 
C<$c-E<gt>req-E<gt>param('language')> takes precedence if both are provided. 
...
file (eg. mydlje-site.development.yaml).

The current domain is detected from C<$ENV{HTTP_HOST}> 
(C<$c-E<gt>req-E<gt>headers-E<gt>host>).

The page is retreived from table C<pages> by alias. 
The passed alias is taken from C<$c-E<gt>stash('alias')>. 
It is put there by Mojolicious. 
If the page has its own template it will be used to render the page. 
Otherwise the table is searched recursively for C<pid> with template.
If no parent page with template is found the default page 
(usually named "home") template is used.
Finally C<page.html.tt> is used as fallback.
The page template specifies the template wrapper, 
which is the layout for the page to be used.
