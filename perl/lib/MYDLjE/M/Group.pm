...
has COLUMNS => sub {
  [ qw(
      id name description namespaces created_by changed_by disabled start stop properties
      )
  ];
};
has FIELDS_VALIDATION => sub {
  ...
  my $fields = {
    ...
    namespaces => {required => 0, constraints => [{length => [0, 255]},]},
...
sub add {
  ...
  unless ($eval_ok) {
    ...
    Carp::croak("ERROR adding group(rolling back):[$@]");
  }
...
MYDLjE::M::Group - MYDLjE::M-based Group class

=head1 SYNOPSIS

  my $group = MYDLjE::M::Group->select(name=>'guest');

=head1 DESCRIPTION

This class is used to instantiate group objects. 
...
in your SQL queries so you do not have to fetch needlessly data from the database and then check 
