...
function scripts_are_executable(){
  ...
  $.ajax({
    ...
    $('#system_check_div ul.oks').append(
      '<li>'+ scripts[i] + '/hi.html : ' + succsess_code + ', '+data + '</li>'  
      );
    },
    error: function(jqXHR, textStatus, errorThrown) {
      $('#system_check_div div.noks').show('slow');
      $('#system_check_div ul.noks').append(
        '<li>'+ scripts[i] + '/hi.html : ' + textStatus +  ', '+ errorThrown +  '</li>'  
      );
...
$(window).load(function(){
  ...
  // Accordion
  $('#setup').accordion({ header: 'h2',autoHeight: false });
